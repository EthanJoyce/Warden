...
public class ZopfliDeflate {
  ...
  static int calculateBlockSize(ZopfliCookie cookie, char[] litLens, char[] dists, int lStart, int lEnd) {
    ...
    System.arraycopy(ZopfliCookie.intZeroes, 0, dLengths, 0, 32);

    int result = 3;

    int[] llCounts = cookie.i288b;
    System.arraycopy(ZopfliCookie.intZeroes, 0, llCounts, 0, 288);
    int[] dCounts = cookie.i32b;
    System.arraycopy(ZopfliCookie.intZeroes, 0, dCounts, 0, 32);
    ...
    ZopfliKatajainen.lengthLimitedCodeLengths(cookie, llCountsCopy, 15, llLengths);

    int[] dCountsCopy = cookie.i32c;
    System.arraycopy(dCounts, 0, dCountsCopy, 0, 32);
    optimizeHuffmanForRle(cookie, dCountsCopy);
    ZopfliKatajainen.lengthLimitedCodeLengths(cookie, dCountsCopy, 15, dLengths);
  ...
  private static void encodeTree(ZopfliCookie cookie, int[] llLengths, int[] dLengths,
                                 boolean use16, boolean use17, boolean use18, ZopfliBuffer output) {
    int hLit = 29;
    int hDist = 29;
    ...
    System.arraycopy(ZopfliCookie.intZeroes, 0, clCounts, 0, 19);
    for (int i = 0; i < rleSize; ++i) {
      clCounts[rle[i]]++;
    }

    int[] clCl = cookie.i19b;
    System.arraycopy(ZopfliCookie.intZeroes, 0, clCl, 0, 19);
    ZopfliKatajainen.lengthLimitedCodeLengths(cookie, clCounts, 7, clCl);
    int[] clSymbols = cookie.i19c;
    lengthsToSymbols(clCl, 19, 7, clSymbols, cookie.i16a, cookie.i16b);

    int[] order = ZopfliUtil.ORDER;
    int hcLen = 15;
  ...
  // TODO: GetRid of RLE
  private static int simulateEncodeTree(ZopfliCookie cookie, int[] llLengths, int[] dLengths,
  ...
  private static void addLzBlock(ZopfliCookie cookie, BlockType type, boolean last, char[] litLens, char[] dists,
    ...
    if (type == BlockType.FIXED) {
      ...
      ZopfliKatajainen.lengthLimitedCodeLengths(cookie, llCounts, 15, llLengths);
      optimizeHuffmanForRle(cookie, dCounts);
      ZopfliKatajainen.lengthLimitedCodeLengths(cookie, dCounts, 15, dLengths);
      patchDistanceCodesForBuggyDecoders(dLengths);
      addDynamicTree(cookie, llLengths, dLengths, output);
    }

    int[] llSymbols = cookie.i288c;
    System.arraycopy(ZopfliCookie.intZeroes, 0, llSymbols, 0, 288);
    lengthsToSymbols(llLengths, 288, 15, llSymbols, cookie.i16a, cookie.i16b);

    int[] dSymbols = cookie.i32b;
    System.arraycopy(ZopfliCookie.intZeroes, 0, dSymbols, 0, 32);
  ...
  private static void optimizeHuffmanForRle(ZopfliCookie cookie, int[] counts) {
    ...
    System.arraycopy(ZopfliCookie.intZeroes, 0, goodForRle, 0, length + 1);
